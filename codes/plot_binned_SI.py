...
binned_results = np.zeros((len(array),4))
...
for chunk in range(0, len(array)):
    ...
    # try:
    #     stat, p_value_normality = stats.shapiro(si_values_in_bin)
    #     normality_results[chunk+1] = p_value_normality
    #     print(f"Shapiro-Wilk normality test p-value: {p_value_normality}")
    # except Exception as Exception:
    #     normality_results[chunk+1] = float('nan')
    #     print(f"Shapiro-Wilk normality test p-value: NaN ")


    # # Perform Kolmogorov-Smirnov test for uniformity
    # try:
    #     ks_stat, p_value_ks = stats.ks_2samp(si_values_in_bin, np.random.uniform(low=min(si_values_in_bin), high=max(si_values_in_bin), size=len(si_values_in_bin)))
    #     uniformity_results[chunk+1] = p_value_ks
    #     print(f"Kolmogorov-Smirnov test p-value: {p_value_ks}")
    # except Exception as Exception:
    #     uniformity_results[chunk+1] = float('nan')
    #     print(f"Kolmogorov-Smirnov test p-value: NaN ")
    ...
    binned_results[chunk,:] = chunk,mean,weight,array[chunk]+bin_width/2
...
# plt.ylim(-2,2)
plt.plot(binned_results[:,3],binned_results[:,1],'o',markersize=30)
...
plt.savefig('../data/sample/'+station+'/SI_binned_measurements_apr8.png',dpi=200)
