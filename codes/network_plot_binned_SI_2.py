...
and binning all measurements within one group! not across different ones due to assumption about all sampling one layer of anisotropy
...
data_tag = 'TAM'
station = 'TAM'
eventdataa = pd.read_csv('/work/gcl3/BR/nicolasv03/senior_thesis/data/'+data_type+'/'+station+'/'+data_tag+'_events_measurements_may30.txt',sep='|')

# df['my_column'] = df['my_column'].fillna('default_value')

eventdataa['evphase'] = eventdataa['evphase'].fillna('SKS')
# data_tag = 'swp'
# station = 'swp'
...
for station_name in station_names:
    ...
    try:
        ...
        # eventdata = eventdataa[eventdataa['evstation']==stat].copy()
        ...
        snr_threshold = 4
        snrbefore_threshold = 2.5
        snrafter_threshold = 2.5
        sta_lta_threshold = 6
        ellipticity_threshold = 50
        mag_threshold = 7.0
        sierrmax = 0.4
        ...
        # eventdata = eventdata[abs(eventdata['wiggleSI_calc'])<=2]
        ...
        binned_results = np.zeros((len(array),5))
        ...
        for chunk in range(0, len(array)):
            ...
            if n <= bin_minimum:
                binned_results[chunk,:] = chunk,0,0,0,0
                continue
            ...
            binned_results[chunk,:] = chunk,mean,1*weight,array[chunk]+bin_width/2,margin 
        ...
        binned_results = binned_results[binned_results[:,2] != 0, :]
        ...
        waveform_ticker = waveform_ticker + len(eventdata)

        # if station_name == 'TOO':
        #     print(len(eventdata))
        #     print('this is TOO btw!')
        #     break
        ...
        # plt.colorbar()
        if single: 
            plt.plot(degs , SI_function(degs, *params),color='black',linewidth=5)
            plt.plot(degs , 2* SI_function(degs, *params),color='red',linewidth=5)          #parameters calculated from binned back azimuths 
          #parameters calculated from binned back azimuths 
        ...
        plt.plot(binned_results[:,3],binned_results[:,1],markersize=10,color='blue',markeredgecolor='blue',marker='s',linestyle='')
        plt.errorbar(binned_results[:,3],binned_results[:,1],yerr=binned_results[:,4],linestyle='',xerr=None,ecolor='blue',markeredgecolor='blue',barsabove=True)

        # histdata = eventdata[(eventdata['calcbaz']>50) & (eventdata['calcbaz']<100)]
        # plt.hist(histdata['wiggleSI_calc'],bins=10)
        ...
        # plt.show()
        # plt.savefig('../data/complete/'+station+'/KA_may_15_'+station_name+'_binned_SI_measurements.png',dpi=200,bbox_inches='tight')
        # plt.savefig('../data/complete/'+station+'/KA_may_15_diagnostic_binned_SI_measurements.png',dpi=200,bbox_inches='tight')
        ...
        plt.savefig('../data/sample/'+station+'/SI_'+station_name+'_deviation_measurements2.png',dpi=200,bbox_inches='tight')
        # #####
...
# saved_df.to_csv(output_dir+'_KA_saved_SI_measurments_may15.txt', sep='|', index=False)
print(waveform_ticker)
